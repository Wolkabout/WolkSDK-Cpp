...
void json_uri::update(const std::string& uri)
{
    ...
    if (pointer_separator != std::string::npos)
    {    // and extract the pointer-string if found
    ...
    if (location.size())
    {    // a location part has been found
        ...
        {    // it is an URL
            ...
            {    // extract the protocol
                ...
                {    // and the hostname (no proto without hostname)
            ...
            {    // the URL contained only a path and the current path has no / at the end, strip last element until /
                 // and append
...
{
    std::vector<std::pair<std::string, std::string>> chars = {{"~", "~0"}, {"/", "~1"}};
