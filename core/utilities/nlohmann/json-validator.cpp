 ...
 *
 */
#include "core/utilities/nlohmann/json-patch.hpp"
    ...
    std::shared_ptr<schema> get_or_create_ref(const json_uri& uri)
    {
        ...
        if (uri.pointer() != "")
        {
            try
            {
                ...
                auto s =
                  schema::make(subschema, this, {}, {{uri}});    //  A JSON Schema MUST be an object or a boolean.
                if (s)
...
template <enum logical_combination_types combine_logic> class logical_combination : public schema
{
    ...
    static bool is_validate_complete(const json&, const json::json_pointer&, error_handler&, const first_error_handler&,
                                     size_t);
        ...
        for (auto& p : instance.items())
        {
            ...
            if (!a_prop_or_pattern_matched && additionalProperties_)
            {
                ...
                if (additional_prop_err)
                    e.error(
                      ptr, instance,
                      "validation failed for additional property '" + p.key() + "': " + additional_prop_err.message_);
            }
        }
